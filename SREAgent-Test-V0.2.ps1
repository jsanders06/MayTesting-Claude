...
# ─── Assemble tabs ────────────────────────────────────────────────────────────
$tabs.TabPages.AddRange(@($tabH,$tabEv,$tabNet,$tabPT,$tabPerm,$tabSvc,$tabSec))
...
function Load-Services {
    ...
    if($filter -ne 'All'){$svcs=@($svcs|Where-Object{$_.Status.ToString() -eq $filter})}
    if($srch){$svcs=@($svcs|Where-Object{$_.Name -like "*$srch*" -or $_.DisplayName -like "*$srch*"})}
